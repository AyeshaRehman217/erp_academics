...
@Service
public class SeederLectureDeliveryModeService {
    ...
    public Mono<ServerResponse> seedLectureDeliveryMode(){

        List<MultiValueMap<String, String>> formDataList = new ArrayList<>();

        MultiValueMap<String, String> formData = new LinkedMultiValueMap<>();
        formData.add("name", "Onsite");
        formData.add("description", "OnSite Lecture Delivery Mode");
        formData.add("status", "true");

        MultiValueMap<String, String> formData1 = new LinkedMultiValueMap<>();
        formData1.add("name", "Online");
        formData1.add("description", "Online Lecture Delivery Mode");
        ...
        for (int i = 0; i < formDataList.size(); i++) {
            Mono<Boolean> res = seederService
                    .seedData(academicBaseURI+"api/v1/lecture-delivery-modes/store", formDataList.get(i));
