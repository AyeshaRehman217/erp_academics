...
import tuf.webscaf.app.dbContext.master.repositry.StudentSiblingAcademicHistoryRepository;
...
import tuf.webscaf.app.dbContext.slave.repositry.SlaveStudentSiblingAcademicHistoryRepository;
...
@Tag(name = "studentSiblingAcademicHistoryHandler")
@Component
public class StudentSiblingAcademicHistoryHandler {
    @Value("${server.zone}")
    private String zone;
    ...
    @Autowired
    SlaveStudentSiblingAcademicHistoryRepository slaveStudentSiblingAcademicHistoryRepository;
    ...
    @AuthHasPermission(value = "academic_api_v1_student-sibling-academic-histories_index")
    public Mono<ServerResponse> index(ServerRequest serverRequest) {
        ...
        if (!status.isEmpty() && !studentSiblingUUID.isEmpty()) {
            ...
            Flux<SlaveStudentSiblingAcademicHistoryEntity> slaveStudentSiblingAcademicHistoryFlux = slaveStudentSiblingAcademicHistoryRepository
                    .findAllByGradeContainingIgnoreCaseAndStudentSiblingUUIDAndStatusAndDeletedAtIsNull(pageable, searchKeyWord, UUID.fromString(studentSiblingUUID), Boolean.valueOf(status));
            return slaveStudentSiblingAcademicHistoryFlux
                    .collectList()
                    .flatMap(studentSiblingAcademicHistoryEntity -> slaveStudentSiblingAcademicHistoryRepository
                            .countByGradeContainingIgnoreCaseAndStudentSiblingUUIDAndStatusAndDeletedAtIsNull(searchKeyWord, UUID.fromString(studentSiblingUUID), Boolean.valueOf(status))
                            .flatMap(count -> {
                                if (studentSiblingAcademicHistoryEntity.isEmpty()) {
                                    return responseIndexInfoMsg("Record does not exist", count);
                                } else {
                                    return responseIndexSuccessMsg("All Records Fetched Successfully", studentSiblingAcademicHistoryEntity, count);
                                }
                            })
            ...
            Flux<SlaveStudentSiblingAcademicHistoryEntity> slaveStudentSiblingAcademicHistoryFlux = slaveStudentSiblingAcademicHistoryRepository
                    .findAllByGradeContainingIgnoreCaseAndStudentSiblingUUIDAndDeletedAtIsNull(pageable, searchKeyWord, UUID.fromString(studentSiblingUUID));
            return slaveStudentSiblingAcademicHistoryFlux
                    .collectList()
                    .flatMap(studentSiblingAcademicHistoryEntity -> slaveStudentSiblingAcademicHistoryRepository
                            .countByGradeContainingIgnoreCaseAndStudentSiblingUUIDAndDeletedAtIsNull(searchKeyWord, UUID.fromString(studentSiblingUUID))
                            .flatMap(count -> {
                                if (studentSiblingAcademicHistoryEntity.isEmpty()) {
                                    return responseIndexInfoMsg("Record does not exist", count);
                                } else {
            ...
            return slaveStudentSiblingAcademicHistoryFlux
                    .collectList()
                    .flatMap(studentSiblingAcademicHistoryEntity -> slaveStudentSiblingAcademicHistoryRepository.countByGradeContainingIgnoreCaseAndDeletedAtIsNull(searchKeyWord)
                            .flatMap(count -> {
                                if (studentSiblingAcademicHistoryEntity.isEmpty()) {
                                    return responseIndexInfoMsg("Record does not exist", count);
                                } else {
    ...
    @AuthHasPermission(value = "academic_api_v1_student-sibling-academic-histories_show")
    public Mono<ServerResponse> show(ServerRequest serverRequest) {
        UUID studentSiblingAcademicHistoryUUID = UUID.fromString(serverRequest.pathVariable("uuid"));

        return slaveStudentSiblingAcademicHistoryRepository.findByUuidAndDeletedAtIsNull(studentSiblingAcademicHistoryUUID)
                .flatMap(studentSiblingAcademicHistoryEntity -> responseSuccessMsg("Record Fetched Successfully", studentSiblingAcademicHistoryEntity))
                .switchIfEmpty(responseInfoMsg("Record does not exist"))
                .onErrorResume(err -> responseErrorMsg("Record does not exist.Please Contact Developer."));
    }

    //This function is used by delete function of Country Handler in Config Module to Check If country Exists in Student Academic History
    @AuthHasPermission(value = "academic_api_v1_student-sibling-academic-histories_country_show")
    public Mono<ServerResponse> getCountryUUID(ServerRequest serverRequest) {
        ...
        return slaveStudentSiblingAcademicHistoryRepository.findFirstByCountryUUIDAndDeletedAtIsNull(countryUUID)
                .flatMap(value1 -> responseInfoMsg("Unable to Delete Record as the Reference Exists."))
                .switchIfEmpty(responseErrorMsg("Record Does not exist"))
                .onErrorResume(ex -> responseErrorMsg("Record Does not exist. Please contact developer."));
    }

    //This function is used by delete function of State Handler in Config Module to Check If state Exists in Student Academic History
    @AuthHasPermission(value = "academic_api_v1_student-sibling-academic-histories_state_show")
    public Mono<ServerResponse> getStateUUID(ServerRequest serverRequest) {
        ...
        return slaveStudentSiblingAcademicHistoryRepository.findFirstByStateUUIDAndDeletedAtIsNull(stateUUID)
                .flatMap(value1 -> responseInfoMsg("Unable to Delete Record as the Reference Exists."))
                .switchIfEmpty(responseErrorMsg("Record Does not exist"))
                .onErrorResume(ex -> responseErrorMsg("Record Does not exist. Please contact developer."));
    }

    //This function is used by delete function of City Handler in Config Module to Check If city Exists in Student Academic History
    @AuthHasPermission(value = "academic_api_v1_student-sibling-academic-histories_city_show")
    public Mono<ServerResponse> getCityUUID(ServerRequest serverRequest) {
        ...
        return slaveStudentSiblingAcademicHistoryRepository.findFirstByCityUUIDAndDeletedAtIsNull(cityUUID)
                .flatMap(value1 -> responseInfoMsg("Unable to Delete Record as the Reference Exists."))
                .switchIfEmpty(responseErrorMsg("Record Does not exist"))
                .onErrorResume(ex -> responseErrorMsg("Record Does not exist. Please contact developer."));
    }

    @AuthHasPermission(value = "academic_api_v1_student-sibling-academic-histories_store")
    public Mono<ServerResponse> store(ServerRequest serverRequest) {
        ...
        return serverRequest.formData()
                .flatMap(value -> {
                    ...
                    StudentSiblingAcademicHistoryEntity entity = StudentSiblingAcademicHistoryEntity.builder()
                    ...
                    //checks if student sibling academic history record already exists
                    return studentSiblingAcademicHistoryRepository
                            .findFirstByDegreeUUIDAndStudentSiblingUUIDAndDeletedAtIsNull(entity.getDegreeUUID(), entity.getStudentSiblingUUID())
                            ...
                            .switchIfEmpty(Mono.defer(() -> studentSiblingRepository.findByUuidAndDeletedAtIsNull(entity.getStudentSiblingUUID())
                                    .flatMap(studentSiblingEntity -> degreeRepository.findByUuidAndDeletedAtIsNull(entity.getDegreeUUID())
                                            //checks city uuid exists
                                            .flatMap(slaveStudentSiblingDocumentEntity -> apiCallService.getDataWithUUID(configUri + "api/v1/cities/show/", entity.getCityUUID())
                                                                    ...
                                                                    .flatMap(stateEntity -> apiCallService.getUUID(cityEntity)
                                                                            ...
                                                                            .flatMap(stateJsonNode -> apiCallService.getDataWithUUID(configUri + "api/v1/countries/show/", entity.getCountryUUID())
                                                                                    .flatMap(countryEntity -> apiCallService.getUUID(countryEntity)
                                                                                            .flatMap(countryJsonNode -> studentSiblingAcademicHistoryRepository.save(entity)
                                                                                                    .flatMap(studentSiblingAcademicHistoryEntity -> responseSuccessMsg("Record Stored Successfully", getResponseEntity(studentSiblingAcademicHistoryEntity, encodedGrade)))
    ...
    public StudentSiblingAcademicHistoryEntity getResponseEntity(StudentSiblingAcademicHistoryEntity entity, String encodedGrade) {
        // set the encoded value
        entity.setGrade(encodedGrade);
        return entity;
    }

    @AuthHasPermission(value = "academic_api_v1_student-sibling-academic-histories_update")
    public Mono<ServerResponse> update(ServerRequest serverRequest) {
        ...
        return serverRequest.formData()
                .flatMap(value -> studentSiblingAcademicHistoryRepository.findByUuidAndDeletedAtIsNull(studentSiblingAcademicHistoryUUID)
                        .flatMap(entity -> {
                            ...
                            StudentSiblingAcademicHistoryEntity updatedEntity = StudentSiblingAcademicHistoryEntity.builder()
                                    .uuid(entity.getUuid())
                                    .status(Boolean.valueOf(value.getFirst("status")))
                            ...
                            //checks if student sibling academic history record already exists
                            return studentSiblingAcademicHistoryRepository
                                    .findFirstByDegreeUUIDAndStudentSiblingUUIDAndDeletedAtIsNullAndUuidIsNot(updatedEntity.getDegreeUUID(), updatedEntity.getStudentSiblingUUID(), studentSiblingAcademicHistoryUUID)
                                    .flatMap(recordAlreadyExists -> responseInfoMsg("Record already exist with same degree"))
                                    //checks if student sibling profile uuid exists
                                    .switchIfEmpty(Mono.defer(() -> studentSiblingRepository.findByUuidAndDeletedAtIsNull(updatedEntity.getStudentSiblingUUID())
                                            .flatMap(studentSiblingEntity -> degreeRepository.findByUuidAndDeletedAtIsNull(updatedEntity.getDegreeUUID())
                                                    //checks city uuid exists
                                                    .flatMap(studentSiblingDocumentEntity -> apiCallService.getDataWithUUID(configUri + "api/v1/cities/show/", updatedEntity.getCityUUID())
                                                            .flatMap(cityEntity -> apiCallService.getUUID(cityEntity)
                                                                    //checks state uuid exists
                                                                    .flatMap(cityJsonNode -> apiCallService.getDataWithUUID(configUri + "api/v1/states/show/", updatedEntity.getStateUUID())
                                                                            .flatMap(stateEntity -> apiCallService.getUUID(cityEntity)
                                                                                    ...
                                                                                    .flatMap(stateJsonNode -> apiCallService.getDataWithUUID(configUri + "api/v1/countries/show/", updatedEntity.getCountryUUID())
                                                                                            .flatMap(countryEntity -> apiCallService.getUUID(countryEntity)
                                                                                                    .flatMap(countryJsonNode -> studentSiblingAcademicHistoryRepository.save(entity)
                                                                                                            .then(studentSiblingAcademicHistoryRepository.save(updatedEntity))
                                                                                                            .flatMap(studentSiblingAcademicRecordEntity -> responseSuccessMsg("Record Updated Successfully", getResponseEntity(studentSiblingAcademicRecordEntity, encodedGrade)))
                                                                    ...
                                                                    .onErrorResume(ex -> responseErrorMsg("City does not exist. Please contact developer.")))
                                                    ).switchIfEmpty(responseInfoMsg("Degree record does not exist")).onErrorResume(ex -> responseErrorMsg("Degree record  does not exist. Please contact developer"))
                                            ).switchIfEmpty(responseInfoMsg("Student Sibling  record does not exist.")).onErrorResume(ex -> responseErrorMsg("Student Sibling  record does not exist. Please contact developer"))
                                    ));
    ...
    @AuthHasPermission(value = "academic_api_v1_student-sibling-academic-histories_status_update")
    public Mono<ServerResponse> status(ServerRequest serverRequest) {
        ...
        return serverRequest.formData()
                .flatMap(value -> {
                    ...
                    return studentSiblingAcademicHistoryRepository.findByUuidAndDeletedAtIsNull(studentSiblingAcademicHistoryUUID)
                            .flatMap(val -> {
                                ...
                                return studentSiblingAcademicHistoryRepository.save(val)
                                        .then(studentSiblingAcademicHistoryRepository.save(entity))
    ...
    @AuthHasPermission(value = "academic_api_v1_student-sibling-academic-histories_delete")
    public Mono<ServerResponse> delete(ServerRequest serverRequest) {
        ...
        return studentSiblingAcademicHistoryRepository.findByUuidAndDeletedAtIsNull(studentSiblingAcademicHistoryUUID)
                .flatMap(studentSiblingAcademicHistoryEntity -> {

                    studentSiblingAcademicHistoryEntity.setDeletedBy(UUID.fromString(userId));
                    studentSiblingAcademicHistoryEntity.setDeletedAt(LocalDateTime.now(ZoneId.of(zone)));
                    studentSiblingAcademicHistoryEntity.setReqCompanyUUID(UUID.fromString(reqCompanyUUID));
                    studentSiblingAcademicHistoryEntity.setReqBranchUUID(UUID.fromString(reqBranchUUID));
                    studentSiblingAcademicHistoryEntity.setReqDeletedIP(reqIp);
                    studentSiblingAcademicHistoryEntity.setReqDeletedPort(reqPort);
                    studentSiblingAcademicHistoryEntity.setReqDeletedBrowser(reqBrowser);
                    studentSiblingAcademicHistoryEntity.setReqDeletedOS(reqOs);
                    studentSiblingAcademicHistoryEntity.setReqDeletedDevice(reqDevice);
                    studentSiblingAcademicHistoryEntity.setReqDeletedReferer(reqReferer);

                    return studentSiblingAcademicHistoryRepository.save(studentSiblingAcademicHistoryEntity)
