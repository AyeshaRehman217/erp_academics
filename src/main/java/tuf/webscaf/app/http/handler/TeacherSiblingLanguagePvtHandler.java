...
import tuf.webscaf.app.dbContext.master.repositry.TeacherSiblingLanguagePvtRepository;
import tuf.webscaf.app.dbContext.master.repositry.TeacherSiblingRepository;
import tuf.webscaf.app.dbContext.slave.repositry.SlaveTeacherSiblingLanguagePvtRepository;
...
@Component
@Tag(name = "teacherSiblingLanguagePvtHandler")
public class TeacherSiblingLanguagePvtHandler {
    @Value("${server.zone}")
    private String zone;

    @Autowired
    TeacherSiblingLanguagePvtRepository teacherSiblingLanguagePvtRepository;

    @Autowired
    SlaveTeacherSiblingLanguagePvtRepository slaveTeacherSiblingLanguagePvtRepository;
    ...
    //This Function Fetch Mapped Language UUID's against Teacher Sibling from Pvt
    @AuthHasPermission(value = "academic_api_v1_teacher-sibling-languages_list_show")
    public Mono<ServerResponse> showList(ServerRequest serverRequest) {
        ...
        return slaveTeacherSiblingLanguagePvtRepository.getAllMappedLanguageUUIDAgainstTeacherSibling(teacherSiblingUUID)
                .flatMap(uuids -> {
    ...
    @AuthHasPermission(value = "academic_api_v1_teacher-sibling-languages_store")
    public Mono<ServerResponse> store(ServerRequest serverRequest) {
        ...
        return serverRequest.formData()
                .flatMap(value -> teacherSiblingRepository.findByUuidAndDeletedAtIsNull(teacherSiblingUUID)
                                ...
                                return apiCallService.getListWithQueryParams(configUri + "api/v1/languages/list/show", "uuid", listOfLanguageUUID)
                                        .flatMap(languageJsonNode -> {
                                            ...
                                            if (!languageList.isEmpty()) {
                                                ...
                                                List<TeacherSiblingLanguagePvtEntity> listPvt = new ArrayList<>();

                                                return teacherSiblingLanguagePvtRepository.findAllByTeacherSiblingUUIDAndLanguageUUIDInAndDeletedAtIsNull(teacherSiblingUUID, languageList)
                                                        .collectList()
                                                        .flatMap(teacherSiblingPvtEntity -> {
                                                            for (TeacherSiblingLanguagePvtEntity pvtEntity : teacherSiblingPvtEntity) {
                                                            ...
                                                            // iterate Language UUIDs for given Teacher Sibling
                                                            for (UUID languageUUID : languageList) {
                                                                TeacherSiblingLanguagePvtEntity voucherLanguagePvtEntity = TeacherSiblingLanguagePvtEntity
                                                                        .builder()
                                                            ...
                                                            return teacherSiblingLanguagePvtRepository.saveAll(listPvt)
                                                                    .collectList()
                                                                    .flatMap(groupList -> {
                        ...
                        }).switchIfEmpty(responseInfoMsg("Teacher Sibling does not exist"))
                        .onErrorResume(err -> responseInfoMsg("Teacher Sibling does not exist. Please contact developer."))
                ).switchIfEmpty(responseInfoMsg("Unable to read request"))
                .onErrorResume(ex -> responseErrorMsg("Unable to read request. Please contact developer."));
    }

    @AuthHasPermission(value = "academic_api_v1_teacher-sibling-languages_delete")
    public Mono<ServerResponse> delete(ServerRequest serverRequest) {
        ...
        return teacherSiblingRepository.findByUuidAndDeletedAtIsNull(teacherSiblingUUID)
                .flatMap(teacherSiblingEntity -> apiCallService.getDataWithUUID(configUri + "api/v1/languages/show/", languageUUID)
                        .flatMap(languageJson -> apiCallService.getUUID(languageJson)
                                .flatMap(language -> teacherSiblingLanguagePvtRepository.findFirstByTeacherSiblingUUIDAndLanguageUUIDAndDeletedAtIsNull(teacherSiblingUUID, languageUUID)
                                        .flatMap(teacherSiblingLanguagePvtEntity -> {

                                            teacherSiblingLanguagePvtEntity.setDeletedAt(LocalDateTime.now());
                                            teacherSiblingLanguagePvtEntity.setDeletedBy(UUID.fromString(userId));
                                            teacherSiblingLanguagePvtEntity.setReqCompanyUUID(UUID.fromString(reqCompanyUUID));
                                            teacherSiblingLanguagePvtEntity.setReqBranchUUID(UUID.fromString(reqBranchUUID));
                                            teacherSiblingLanguagePvtEntity.setReqDeletedIP(reqIp);
                                            teacherSiblingLanguagePvtEntity.setReqDeletedPort(reqPort);
                                            teacherSiblingLanguagePvtEntity.setReqDeletedBrowser(reqBrowser);
                                            teacherSiblingLanguagePvtEntity.setReqDeletedOS(reqOs);
                                            teacherSiblingLanguagePvtEntity.setReqDeletedDevice(reqDevice);
                                            teacherSiblingLanguagePvtEntity.setReqDeletedReferer(reqReferer);

                                            return teacherSiblingLanguagePvtRepository.save(teacherSiblingLanguagePvtEntity)
