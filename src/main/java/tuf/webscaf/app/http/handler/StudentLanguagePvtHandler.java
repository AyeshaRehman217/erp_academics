...
import tuf.webscaf.app.dbContext.master.repositry.StudentLanguagePvtRepository;
import tuf.webscaf.app.dbContext.master.repositry.StudentRepository;
import tuf.webscaf.app.dbContext.slave.repositry.SlaveStudentLanguagePvtRepository;
...
@Component
@Tag(name = "studentLanguagePvtHandler")
public class StudentLanguagePvtHandler {
    @Value("${server.zone}")
    private String zone;
    ...
    @Autowired
    SlaveStudentLanguagePvtRepository slaveStudentLanguagePvtRepository;
    ...
    //This Function Fetch Mapped Language UUID's against Student from Pvt
    @AuthHasPermission(value = "academic_api_v1_student-languages_list_show")
    public Mono<ServerResponse> showList(ServerRequest serverRequest) {
        ...
        return slaveStudentLanguagePvtRepository.getAllMappedLanguageUUIDAgainstStudent(studentUUID)
                .flatMap(uuids -> {
    ...
    @AuthHasPermission(value = "academic_api_v1_student-languages_store")
                                ...
                                return apiCallService.getListWithQueryParams(configUri + "api/v1/languages/list/show", "uuid", listOfLanguageUUID)
                                        .flatMap(languageJsonNode -> {
                                            ...
                                            if (!languageList.isEmpty()) {
                                                ...
                                                List<StudentLanguagePvtEntity> listPvt = new ArrayList<>();

                                                return studentLanguagePvtRepository.findAllByStudentUUIDAndLanguageUUIDInAndDeletedAtIsNull(studentUUID, languageList)
                                                        .collectList()
                                                        .flatMap(studentPvtEntity -> {
                                                            for (StudentLanguagePvtEntity pvtEntity : studentPvtEntity) {
                                                            ...
                                                            // iterate Language UUIDs for given Student
                                                            for (UUID languageUUID : languageList) {

                                                                StudentLanguagePvtEntity voucherLanguagePvtEntity = StudentLanguagePvtEntity
                                                                        .builder()
                                                            ...
                                                            return studentLanguagePvtRepository.saveAll(listPvt)
                                                                    .collectList()
                                                                    .flatMap(groupList -> {
    ...
    @AuthHasPermission(value = "academic_api_v1_student-languages_delete")
    public Mono<ServerResponse> delete(ServerRequest serverRequest) {
        ...
        return studentRepository.findByUuidAndDeletedAtIsNull(studentUUID)
                .flatMap(studentEntity -> apiCallService.getDataWithUUID(configUri + "api/v1/languages/show/", languageUUID)
                        .flatMap(languageJson -> apiCallService.getUUID(languageJson)
                                .flatMap(language -> studentLanguagePvtRepository.findFirstByStudentUUIDAndLanguageUUIDAndDeletedAtIsNull(studentUUID, languageUUID)
                                        .flatMap(studentLanguagePvtEntity -> {

                                            studentLanguagePvtEntity.setDeletedAt(LocalDateTime.now());
                                            studentLanguagePvtEntity.setDeletedBy(UUID.fromString(userId));
                                            studentLanguagePvtEntity.setReqCompanyUUID(UUID.fromString(reqCompanyUUID));
                                            studentLanguagePvtEntity.setReqBranchUUID(UUID.fromString(reqBranchUUID));
                                            studentLanguagePvtEntity.setReqDeletedIP(reqIp);
                                            studentLanguagePvtEntity.setReqDeletedPort(reqPort);
                                            studentLanguagePvtEntity.setReqDeletedBrowser(reqBrowser);
                                            studentLanguagePvtEntity.setReqDeletedOS(reqOs);
                                            studentLanguagePvtEntity.setReqDeletedDevice(reqDevice);
                                            studentLanguagePvtEntity.setReqDeletedReferer(reqReferer);

                                            return studentLanguagePvtRepository.save(studentLanguagePvtEntity)
