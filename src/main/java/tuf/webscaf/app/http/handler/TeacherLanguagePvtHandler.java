...
import tuf.webscaf.app.dbContext.master.repositry.TeacherLanguagePvtRepository;
import tuf.webscaf.app.dbContext.master.repositry.TeacherRepository;
import tuf.webscaf.app.dbContext.slave.repositry.SlaveTeacherLanguagePvtRepository;
...
@Component
@Tag(name = "teacherLanguagePvtHandler")
public class TeacherLanguagePvtHandler {
    @Value("${server.zone}")
    private String zone;
    ...
    @Autowired
    SlaveTeacherLanguagePvtRepository slaveTeacherLanguagePvtRepository;
    ...
    //This Function Fetch Mapped Language UUID's against Teacher from Pvt
    @AuthHasPermission(value = "academic_api_v1_teacher-languages_list_show")
    public Mono<ServerResponse> showList(ServerRequest serverRequest) {
        ...
        return slaveTeacherLanguagePvtRepository.getAllMappedLanguageUUIDAgainstTeacher(teacherUUID)
                .flatMap(uuids -> {
    ...
    @AuthHasPermission(value = "academic_api_v1_teacher-languages_store")
                                ...
                                return apiCallService.getListWithQueryParams(configUri + "api/v1/languages/list/show", "uuid", listOfLanguageUUID)
                                        .flatMap(languageJsonNode -> {
                                            ...
                                            if (!languageList.isEmpty()) {
                                                ...
                                                List<TeacherLanguagePvtEntity> listPvt = new ArrayList<>();

                                                return teacherLanguagePvtRepository.findAllByTeacherUUIDAndLanguageUUIDInAndDeletedAtIsNull(teacherUUID, languageList)
                                                        .collectList()
                                                        .flatMap(teacherPvtEntity -> {
                                                            for (TeacherLanguagePvtEntity pvtEntity : teacherPvtEntity) {
                                                            ...
                                                            // iterate Language UUIDs for given Teacher
                                                            for (UUID languageUUID : languageList) {
                                                                TeacherLanguagePvtEntity voucherLanguagePvtEntity = TeacherLanguagePvtEntity
                                                                        .builder()
                                                            ...
                                                            return teacherLanguagePvtRepository.saveAll(listPvt)
                                                                    .collectList()
                                                                    .flatMap(groupList -> {
    ...
    @AuthHasPermission(value = "academic_api_v1_teacher-languages_delete")
    public Mono<ServerResponse> delete(ServerRequest serverRequest) {
        ...
        return teacherRepository.findByUuidAndDeletedAtIsNull(teacherUUID)
                .flatMap(teacherEntity -> apiCallService.getDataWithUUID(configUri + "api/v1/languages/show/", languageUUID)
                        .flatMap(languageJson -> apiCallService.getUUID(languageJson)
                                .flatMap(language -> teacherLanguagePvtRepository.findFirstByTeacherUUIDAndLanguageUUIDAndDeletedAtIsNull(teacherUUID, languageUUID)
                                        .flatMap(teacherLanguagePvtEntity -> {

                                            teacherLanguagePvtEntity.setDeletedAt(LocalDateTime.now());
                                            teacherLanguagePvtEntity.setDeletedBy(UUID.fromString(userId));
                                            teacherLanguagePvtEntity.setReqCompanyUUID(UUID.fromString(reqCompanyUUID));
                                            teacherLanguagePvtEntity.setReqBranchUUID(UUID.fromString(reqBranchUUID));
                                            teacherLanguagePvtEntity.setReqDeletedIP(reqIp);
                                            teacherLanguagePvtEntity.setReqDeletedPort(reqPort);
                                            teacherLanguagePvtEntity.setReqDeletedBrowser(reqBrowser);
                                            teacherLanguagePvtEntity.setReqDeletedOS(reqOs);
                                            teacherLanguagePvtEntity.setReqDeletedDevice(reqDevice);
                                            teacherLanguagePvtEntity.setReqDeletedReferer(reqReferer);

                                            return teacherLanguagePvtRepository.save(teacherLanguagePvtEntity)
