...
public class IndexSubjectWithStudentCourseAndSemesterHandlerFilter implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    @Override
    public Mono<ServerResponse> filter(ServerRequest request, HandlerFunction<ServerResponse> next) {
        ...
        //Optional Query Parameter Based of semesterUUID
        String semesterUUID = request.queryParam("semesterUUID").map(String::toString).orElse("").trim();
                ...
                .flatMap(value -> {
                    ...
                    if (!studentUUID.isEmpty()) {
                        ...
                        messages.add(
                                new AppResponseMessage(
                                        AppResponse.Response.INFO,
                                        "Please Enter Student Record"
                                )
                        );
                    }

                    if (!semesterUUID.isEmpty()) {
                        if (!semesterUUID.matches("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")) {
                            messages.add(
                                    new AppResponseMessage(
                                            AppResponse.Response.ERROR,
                                            "Invalid Semester"
