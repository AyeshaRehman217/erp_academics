...
public class ShowStudentChildProfileAgainstChildAndChildHandlerFilter implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    @Override
    public Mono<ServerResponse> filter(ServerRequest request, HandlerFunction<ServerResponse> next) {


        String studentChildUUID = request.queryParam("studentChildUUID").map(String::toString).orElse("");
                    ...
                    if (!request.pathVariable("uuid").matches("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")) {
                        messages.add(
                                new AppResponseMessage(
                                        ...
                                        "Invalid Student Child Profile UUID"
                                )
                        );
                    }
