package tuf.webscaf.app.http.validationFilters.teacherProfileHandler;
...
public class StoreTeacherProfileHandlerFilter implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    @Override
        ...
        return request.formData()
                .flatMap(value -> {
                    ...
                    if (value.containsKey("email")) {
                        if (!value.getFirst("email").equals("") && !value.getFirst("email").equals(null)) {
                    ...
                    if (value.containsKey("emergencyContactNoUUID")) {
                        if (!value.getFirst("emergencyContactNoUUID").matches("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")) {
                            messages.add(
                                    new AppResponseMessage(
                                            AppResponse.Response.ERROR,
                                            "Invalid Emergency Contact No UUID!"
                                    )
                            );
                        }
                    }
