...
public class IndexCloHandlerFilter implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    @Override
    public Mono<ServerResponse> filter(ServerRequest request, HandlerFunction<ServerResponse> next) {
        ...
        // Optional Query Parameter of department UUID
        String departmentUUID = request.queryParam("departmentUUID").map(String::toString).orElse("").trim();

        // Optional Query Parameter of emphasis UUID
        String emphasisUUID = request.queryParam("emphasisUUID").map(String::toString).orElse("").trim();
        ...
        return request.formData()
                .flatMap(value -> {
                    ...
                    if (!emphasisUUID.isEmpty()) {
                        if (!emphasisUUID.matches("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")) {
                            messages.add(
                                    new AppResponseMessage(
                                            AppResponse.Response.ERROR,
                                            "Invalid Emphasis"
