...
public class IndexTimetableChangeReasonHandlerFilter implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    @Override
    public Mono<ServerResponse> filter(ServerRequest request, HandlerFunction<ServerResponse> next) {
        ...
        String timetableUUID = request.queryParam("timetableUUID").map(String::toString).orElse("").trim();
        ...
        return request.formData()
                .flatMap(value -> {
                    ...
                    if (!timetableUUID.isEmpty()) {
                        if (!timetableUUID.matches("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")) {
                            messages.add(
                                    new AppResponseMessage(
