package tuf.webscaf.app.http.validationFilters.studentContactNoHandler;
...
public class UpdateStudentContactNoHandlerFilter implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    @Override
...
//                    }
                    ...
                    if (value.containsKey("studentMetaUUID")) {
                        if (value.getFirst("studentMetaUUID").isEmpty()) {
                            messages.add(
                                    new AppResponseMessage(
                                            AppResponse.Response.ERROR,
                                            "Student Meta Field Required"
                                    )
                            );
                        } else {
                            if (!value.getFirst("studentMetaUUID").matches("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")) {
                                messages.add(
                                        new AppResponseMessage(
                                                AppResponse.Response.ERROR,
                                                "Invalid Student Meta"
                                        )
                                );
                            }
                        }
