package tuf.webscaf.app.http.validationFilters.studentSiblingJobHistoryHandler;
...
public class StoreStudentSiblingJobHistoryHandlerFilter implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    @Override
    public Mono<ServerResponse> filter(ServerRequest request, HandlerFunction<ServerResponse> next) {
        ...
        return request.formData()
                .flatMap(value -> {
                    ...
                    if (value.containsKey("income")) {
                        if (value.getFirst("income").isEmpty()) {
                            messages.add(
                                    new AppResponseMessage(
                                            AppResponse.Response.ERROR,
                                            "Income Field Required"
                                    )
                            );
                        } else {
                            if (!value.getFirst("income").matches("^[0-9]*$")) {
                                messages.add(
                                        new AppResponseMessage(
                                                AppResponse.Response.ERROR,
                                                "Invalid Income.Income Must be Numeric"
                                        )
                                );
                            }
                        }
                    ...
                    if (value.containsKey("studentSiblingUUID")) {
                        if (value.getFirst("studentSiblingUUID").isEmpty()) {
                            messages.add(
                                    new AppResponseMessage(
                                            AppResponse.Response.ERROR,
                                            "Student Sibling UUID Field Required"
                                    )
                            );
                        } else {
