...
public class SlaveCustomHobbyStudentPvtRepositoryImpl implements SlaveCustomHobbyStudentPvtRepository {
    private DatabaseClient client;
    private SlaveHobbyEntity slaveHobbyEntity;

    @Autowired
    public SlaveCustomHobbyStudentPvtRepositoryImpl(@Qualifier("slave") ConnectionFactory cf) {
        this.client = DatabaseClient.create(cf);
    }

    @Override
    public Flux<SlaveHobbyEntity> existingHobbyList(UUID studentUUID, String name, String dp, String d, Integer size, Long page) {
    ...
    @Override
    public Flux<SlaveHobbyEntity> existingStudentHobbiesListWithStatusCheck(UUID studentUUID, String name, String description, Boolean status, String dp, String d, Integer size, Long page) {
        String query = "SELECT hobbies.* FROM hobbies\n" +
                "WHERE hobbies.uuid NOT IN(\n" +
                "SELECT hobbies.uuid FROM hobbies\n" +
                "LEFT JOIN std_hobbies_pvt\n" +
    ...
    @Override
    public Flux<SlaveHobbyEntity> showStudentHobbiesList(UUID studentUUID, String name, String description, String dp, String d, Integer size, Long page) {
        String query = "select hobbies.* from hobbies\n" +
                "left join std_hobbies_pvt \n" +
    ...
    @Override
    public Flux<SlaveHobbyEntity> showStudentHobbiesListWithStatus(UUID studentUUID, Boolean status, String name, String description, String dp, String d, Integer size, Long page) {
        String query = "select hobbies.* from hobbies\n" +
                "left join std_hobbies_pvt \n" +
                "on hobbies.uuid = std_hobbies_pvt.hobby_uuid \n" +
                "where std_hobbies_pvt.student_uuid = '" + studentUUID +
