...
public class SlaveCustomSubjectAgainstSessionAndTeacherMapper implements BiFunction<Row, Object, SlaveSubjectDto> {
    @Override
    public SlaveSubjectDto apply(Row source, Object o) {
        return SlaveSubjectDto.builder()
                .id(source.get("id", Long.class))
                .version(source.get("version", Long.class))
                .uuid(source.get("uuid", UUID.class))
                .openLMS(source.get("openLMS", Boolean.class))
                .status(source.get("status", Boolean.class))
                .key(source.get("key", String.class))
                .course(source.get("courseName", String.class))
